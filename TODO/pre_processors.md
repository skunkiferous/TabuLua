...
**Landed.** The feature described below shipped — see the v0.x range of
[CHANGELOG.md](../CHANGELOG.md) entries that introduced
`preProcessors:{processor_spec}|nil`, the `processor_executor` module,
and the `rerunAfterPatches` plumbing. The dispatch path was further
generalised by the [type_wiring.md](type_wiring.md) registry refactor:
graph-family auto-wired pre-processors (and any future built-in or
user-package contributions) now flow through `type_wiring.applyWiring`
rather than the hand-written dispatch this plan originally proposed.
User-authored `preProcessors` cells in `Files.tsv` are unchanged.

This document is kept as the design reference; what the engine does
today still matches the design below.
