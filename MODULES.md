...
| [patch_executor](#patch_executor) | Tier-A/B mod row patches: applies key-addressed `add`/`remove`/`update`/`replace` (`patch`) and `where`-selected filter/transform (`bulk_patch`) ops to target parent datasets in load order, mutating in place without baking into parent source | named_logger, parsers, read_only, tsv_model, validator_executor |
