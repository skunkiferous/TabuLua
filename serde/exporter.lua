...
-- The DDL half of the SQL format (column-name sanitizer, type mapping,
-- CREATE TABLE/INSERT preamble). A LEAF module, so the .sql content-pipeline
-- transcoder can share these rules without requiring the exporter back --
-- this module requires content.content_pipeline, and that cycle would hand
-- the transcoder a partially-initialized table. See serde/sql_schema.lua.
local sql_schema = require("serde.sql_schema")
local sqlColumnName = sql_schema.sqlColumnName
...
local function exportSQL(process_files, exportParams)
    ...
    -- we encounters.
    local sql_types = sql_schema.newTypeCache()
