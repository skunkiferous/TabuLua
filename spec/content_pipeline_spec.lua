...
describe("content_pipeline", function()
  ...
  describe("autoTranscodes / reversibleTranscode (extension-keyed transcoder)", function()
    it("autoTranscodes is true for an extension-matched transcoder (.eav)", function()
      assert.is_true(content_pipeline.autoTranscodes("Item.eav"))
      assert.is_true(content_pipeline.autoTranscodes("path/to/Foo.EAV"))
    end)

    it("autoTranscodes is false for plain data and id-only transcoders", function()
      assert.is_false(content_pipeline.autoTranscodes("Item.tsv"))
      assert.is_false(content_pipeline.autoTranscodes("data.csv"))
      -- JSON transcoders are id-only (no auto-matcher), so .json does not auto-fire.
      assert.is_false(content_pipeline.autoTranscodes("items.json"))
    end)

    it("reversibleTranscode returns an encoder for the reversible .eav stage", function()
      local rt = content_pipeline.reversibleTranscode("Item.eav")
      assert.is_not_nil(rt)
      assert.is_function(rt.encode)
    end)

    it("reversibleTranscode returns nil for non-transcoded / id-only sources", function()
      assert.is_nil(content_pipeline.reversibleTranscode("Item.tsv"))
      assert.is_nil(content_pipeline.reversibleTranscode("items.json"))
    end)
  end)
end)
