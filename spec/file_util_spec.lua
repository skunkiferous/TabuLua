...
describe("file_util", function()
  ...
  describe("getFilesAndDirs", function()
      ...
      it("should skip excluded directories when recursing", function()
          local keep_dir = path_join(temp_dir, "keep")
          local skip_dir = path_join(temp_dir, "skip")
          local nested = path_join(skip_dir, "nested")
          lfs.mkdir(keep_dir)
          lfs.mkdir(skip_dir)
          lfs.mkdir(nested)
          local file1 = path_join(temp_dir, "root.txt")
          local file2 = path_join(keep_dir, "kept.txt")
          local file3 = path_join(skip_dir, "skipped.txt")
          local file4 = path_join(nested, "deep.txt")
          file_util.writeFile(file1, "c1")
          file_util.writeFile(file2, "c2")
          file_util.writeFile(file3, "c3")
          file_util.writeFile(file4, "c4")

          local normSkip = file_util.normalizePath(skip_dir)
          local excludeDirs = { [normSkip] = true }
          local files, dirs = file_util.getFilesAndDirs(temp_dir, true, nil, excludeDirs)
          table.sort(files)
          assert.same(2, #files)
          assert.is_true(files[1]:find("kept%.txt") ~= nil)
          assert.is_true(files[2]:find("root%.txt") ~= nil)
          assert.same({keep_dir}, dirs)
      end)
  end)
  ...
  describe("collectFiles", function()
      ...
      it("should exclude directories specified in excludeDirs", function()
          local sub_dir = path_join(temp_dir, "sub_dir")
          local exported_dir = path_join(temp_dir, "exported")
          lfs.mkdir(sub_dir)
          lfs.mkdir(exported_dir)
          file_util.writeFile(path_join(temp_dir, "file1.txt"), "content1")
          file_util.writeFile(path_join(sub_dir, "file2.txt"), "content2")
          file_util.writeFile(path_join(exported_dir, "file3.txt"), "content3")

          local normExported = file_util.normalizePath(exported_dir)
          local excludeDirs = { [normExported] = true }
          local files, errors = file_util.collectFiles({temp_dir}, {"txt"}, nil, nil, excludeDirs)
          assert.is_nil(errors)
          assert.same(2, #files)
          local all = table.concat(files, ",")
          assert.is_true(all:find("file1%.txt") ~= nil)
          assert.is_true(all:find("file2%.txt") ~= nil)
          assert.is_nil(all:find("file3%.txt"))
      end)

      it("should exclude nested subdirectories within excluded dirs", function()
          local exported_dir = path_join(temp_dir, "exported")
          local nested_dir = path_join(exported_dir, "json-natural")
          lfs.mkdir(exported_dir)
          lfs.mkdir(nested_dir)
          file_util.writeFile(path_join(temp_dir, "data.txt"), "content")
          file_util.writeFile(path_join(nested_dir, "deep.txt"), "nested")
          ...
          assert.is_nil(errors)
          assert.same(1, #files)
          assert.is_true(files[1]:find("data%.txt") ~= nil)
      end)
  end)
