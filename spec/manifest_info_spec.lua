...
describe("manifest_info", function()
  ...
  describe("versionSatisfies", function()
    ...
    describe("validateVariantGroups", function()
        ...
        it("should fail when no variants provided and no default declared", function()
        ...
        it("should pass with default when no variants provided", function()
            local m = make_manifest({{"lang", {"en", "fr"}, "en"}})
            local ok, defaults = manifest_info.validateVariantGroups(m, nil, badVal)
            assert.is_true(ok)
            assert.is_not_nil(defaults)
            assert.is_true(defaults["en"])
        end)

        it("should apply default when no variant from a group is selected", function()
            local m = make_manifest({{"lang", {"en", "fr"}, "en"}, {"platform", {"ios", "android"}}})
            local ok, defaults = manifest_info.validateVariantGroups(m, {ios = true}, badVal)
            assert.is_true(ok)
            assert.is_not_nil(defaults)
            assert.is_true(defaults["en"])
        end)

        it("should fail when no variant from a group is selected and no default", function()
        ...
        it("should not apply default when an explicit variant from the group is selected", function()
            local m = make_manifest({{"lang", {"en", "fr"}, "en"}})
            local ok, defaults = manifest_info.validateVariantGroups(m, {fr = true}, badVal)
            assert.is_true(ok)
            assert.is_nil(defaults)
        end)
