...
| `content/` | Pre-parse content pipeline + transcoders | content_pipeline, builtin_content_stages, compression, archive_formats, lua_cog, cog_discovery, doc_generator, eav_transcoder, json_transcoders, xml_transcoder, tsv_transcoders, lua_transcoder, sql_transcoder |
...
| [importer](#importer) | File import system for various formats | base64, deserialization, file_util, int64, named_logger, read_only, sql_schema, string_utils |
...
| [builtin_content_stages](#builtin_content_stages) | Seeds the content-pipeline registry with the built-in stages (EOL-normalise, COG macro, gzip decode, JSON + EAV + XML + TSV-cell + Lua-file + SQL transcoders) | compression, content_pipeline, eav_transcoder, file_util, global_reset, json_transcoders, lua_cog, lua_transcoder, read_only, sql_transcoder, tsv_transcoders, xml_transcoder |
...
| [sql_transcoder](#sql_transcoder) | Content-pipeline transcoders reading/writing TabuLua's `--file=sql` export, one per cell encoding (`sql:json-typed` / `sql:json-natural` / `sql:xml` / `sql:mpk`); id-selected, schema-free (the file's embedded `tabulua_schema` table), reversible, pure-Lua (no `lsqlite3`) | deserialization, error_reporting, importer, parsers, raw_tsv, read_only, serialization, sql_schema, tsv_model |
...
**Dependencies:** compression, content_pipeline, eav_transcoder, file_util, global_reset, json_transcoders, lua_cog, lua_transcoder, read_only, sql_transcoder, tsv_transcoders, xml_transcoder
...
The content-stage registry — the sibling of the [type_wiring](#type_wiring) registry. type-wiring dispatches on a file's *parsed* record type; this registry dispatches on a file's *name* (extension / basename glob / directory / magic bytes) **before** any parsing, operating on raw bytes/text. Stages are grouped into ordered phases — `decode` (decompress; loops with extension peeling), `transcode` (structured→TSV; single match, id- or extension-selected), `normalize` (core EOL), `macro` (COG), `asset` (binary→binary) — and each file carries a `text`/`binary` content kind so text-only phases never touch binary. `readAndRun` (read + source pipeline) and `runSink` (export/inverse direction) are the entry points; `register` adds stages (built-ins via [builtin_content_stages](#builtin_content_stages), user stages via the bootstrap api). Helpers `peeledName` / `reversibleDecode` (decode round-trip) and `autoTranscodes` / `reversibleTranscode` (transcode routing + round-trip) support the loader and reformatter. `reversibleTranscode` binds the **file name** into the encoder it returns — `encode(content, env, badVal, file_name)` — because a forward `transform` receives the name and an encoder that must reproduce anything derived from it otherwise cannot (a `.sql` names its table after its file, so a nameless encoder would rename the user's table on every reformat); encoders that do not need it ignore the argument. Snapshots/restores via `global_reset`.
...
It also owns the **embedded `tabulua_schema` metadata table** that makes a `.sql` self-describing, and therefore re-importable. `metadataSQL` emits it — the byte-identical `CREATE TABLE IF NOT EXISTS` DDL, a portable per-table `DELETE`-then-`INSERT`, a `position = 0` table-info row carrying the version marker and the model dataset name, and one row per column with **both** names (`column_name` = the model's `stats.attack`, `sql_name` = the physical `stats_attack`), its `type_spec` and its default. The metadata and the `CREATE TABLE` are built from **one** `exportedColumns()` list, so they cannot drift: "the metadata describes exactly the physical columns" is the invariant the reader checks, and the retired `-- tabulua-types:` comment broke it by construction (it walked the whole header, so it listed comment columns the DDL had already dropped). `SCHEMA_VERSION` is TabuLua's own `major.minor`; `modelDatasetName` peels the `.transposed` infix the way the loader does, which is why `Manifest.transposed.sql` reports the dataset `Manifest`.

`cellSQL` writes one data cell — a bytes column as a `BLOB` literal (hex digits directly, base64 decoded first), everything else through `serialization.serializeSQL` with the caller's `--data` table serialiser — and `tableSQL` assembles a whole file from a header and its rows. Both are shared with the reversible `encode` in [sql_transcoder](#sql_transcoder): the exporter calls them too, so an exported `.sql` and a reformatted one spell the same value the same way. The bytes-column predicates live here for the same reason (the MessagePack export imports them rather than keeping a second copy).

It is a deliberate **leaf**, extracted from [exporter](#exporter) (`TODO/sql_input_round_trip.md` Phase 0) because `.sql` is an *input* format too: a content-pipeline transcoder needs these same rules to *read* a file, and since the exporter requires [content_pipeline](#content_pipeline), a transcoder requiring the exporter back would risk a partially-initialized module. Nothing here requires the exporter, the importer, or the content pipeline — keep it that way.

**Dependencies:** base64, file_util, named_logger, parsers, read_only, serialization
...
The SQL reader is the substantial one, and it is what [sql_transcoder](#sql_transcoder) is built on. `parseSQLContent(content, tableDeserializer)` returns `(rows, err, schema)`: it locates the **data** table by name (the one that is not `tabulua_schema` — "the first `CREATE TABLE`" stopped being an answer once every exported file opens with the metadata DDL, and two data tables is a refusal rather than a guess), reads the embedded schema, and then labels the header with the **model** column names in stored `position` order, locating each value by its stored `sql_name`. Reading by stored name rather than re-running the exporter's sanitizer is what lets a change to that rule not mis-read an older file. The schema also supplies the declared type per **physical** column, which is what tells a `BLOB` holding hex from one holding base64, and a `BIGINT` that is an `int64` box (read from its digits, never through `tonumber`, which is already rounded on LuaJIT) from one that is a plain number. Row 0's `attributes` bag is validated structurally — NULL or a JSON object, size-capped, known `tabulua.*` keys type-checked, everything else preserved and never interpreted. A file with no metadata table still parses, but only under its physical column names and with no declared types; the retired `-- tabulua-types:` comment has **no** fallback.

`importSQLFileWithSQLite` remains an explicit opt-in for a caller that wants a real engine to parse the DDL; it now reports the same model-named, position-ordered header as the text parser.

**Dependencies:** base64, deserialization, file_util, int64, named_logger, read_only, sql_schema, string_utils
...
**Two extension lists decide what a loose file is.** `EXTENSIONS` is what the loader *collects* from disk — a file not in it cannot be declared in `Files.tsv` at all, which is why `.sql` had to join it when `.sql` became an input format. `DATA_EXTENSIONS` is the narrower guess consulted only for a collected file that no declaration covers (fileRole rule 4): a file that *looks* like data must be declared, everything else is treated as an asset and copied through. `.sql` sits with `xml` in that set — both are export formats that became input formats, both need an explicit `transcoder` to be read, but a loose one in a data directory is a dump of the data rather than an asset. (`lua` deliberately stays out: it is a code library by default.)
...
### sql_transcoder
**File:** [sql_transcoder.lua](../content/sql_transcoder.lua)

Content-pipeline `transcode` stages that read back TabuLua's `--file=sql` export (`TODO/sql_input_round_trip.md`). Four of them, because a SQL export has two independent axes: the container is always `CREATE TABLE` + `INSERT`, but each composite cell is encoded by `--data`, and that encoding is **not** detectable from the SQL — so the id names it (`sql:json-typed` / `sql:json-natural` / `sql:xml` / `sql:mpk`), exactly as `tsv:*` does. Id-only with an `inputExtensions={"sql"}` guard: a `.sql` is opted in with `transcoder=sql:<data>` in `Files.tsv` and never auto-interpreted.

This module is an adapter, not a second parser: [importer](#importer)'s `parseSQLContent` does the reading (including the embedded `tabulua_schema` table, which it returns as a third result) and [sql_schema](#sql_schema) does the writing. **Pure-Lua only** — `lsqlite3` is never involved, so a `.sql` loads identically whether or not that rock is installed. **Schema-free**: the header is rebuilt from the file's own metadata table (model names, `type_spec`s and defaults, in stored `position` order), not from a `typeName`; each value is then re-serialised to native cell text through the column's own [parsers](#parsers) parser, so the output is byte-for-byte the wide TSV any other source for that schema would produce. One coercion is needed on the way in — SQL has no boolean, so a `SMALLINT` 1/0 is mapped back for a boolean column.

The reversible `encode` re-parses the reformatted wide TSV with [tsv_model](#tsv_model) and re-emits through `sql_schema.tableSQL`, the same writer [exporter](#exporter)`.exportSQL` assembles, so reformatting an exported `.sql` is **byte-identical** rather than merely equivalent. It is the reason `encode` receives the file name as a 4th argument (bound in by `content_pipeline.reversibleTranscode`): a `.sql` names its table after its file, and an encoder without the name would rename the user's table on every reformat. The round-trip is **normalizing and mildly lossy** — comment *rows* have no SQL representation (defaults do, and survive).

Refuses, naming the reason: a missing or empty `tabulua_schema`; metadata that does not match the data table; two data tables (a whole-export concatenation is a database, not a dataset); a `--collapse-exploded` export (recorded as `tabulua.collapsed`, whose header would be spelled differently from the source); and a malformed literal.

**Dependencies:** deserialization, error_reporting, importer, parsers, raw_tsv, read_only, serialization, sql_schema, tsv_model
